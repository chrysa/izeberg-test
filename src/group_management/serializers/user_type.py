...
class UserTypeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data: dict[str, str]) -> NoReturn | dict[str, str]:
        if UserType.objects.filter(
            user=data.get('user').pk, group_type__pk__in=[group.pk for group in data.get('group_type')]
        ).exists():
